...
function normalizeTargetMealSet(raw) {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return null;
  const topicSummary = safeTrim(raw.topicSummary || raw.versionSummary);
  const mealIdeas = sanitizeMealIdeas(raw.mealIdeas);
  const subjectItems = sanitizeMealIdeas(raw.subjectItems || raw.mealIdeas);
  const activeConstraints = sanitizeConstraints(raw.activeConstraints);
  const groceryFocus = sanitizeMealIdeas(raw.groceryFocus);
  if (!topicSummary && mealIdeas.length === 0 && subjectItems.length === 0) return null;
...
export async function executeMealRefine(runtimeAction, context) {
  ...
  const explicitMealSet = normalizeTargetMealSet(runtimeAction?.input?.targetMealSet);
...
- subjectItems should list only the immediately relevant dishes or recipe targets after the refinement.
...
- If the recent transcript suggests the user is no longer revising the active meal plan and is instead cooking something tonight or recalling a recipe, return {"status":"needs_context","question":"..."} rather than pretending to refine the meal set.
- Do not add optional customization questions unless the missing detail is truly required to revise the plan.
