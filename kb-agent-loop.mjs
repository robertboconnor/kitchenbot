...
function buildLoopSystemPrompt({ memoryContext, name }) {
  ...
  const principles = [
    ...
    'To CHANGE a recipe (add or swap an ingredient, adjust a step, tweak seasoning), YOU do the revision: rewrite the recipe with the change and show the updated version. There is no separate revise tool. If it should be saved: for a recipe already in the cookbook use cookbook.update — identify it by its exact saved title in `name` (use cookbook.list if unsure of the title) and pass the change; for a new or in-chat recipe use cookbook.save with the full revised recipe. Only save when the user wants it saved — otherwise just present the revised recipe.',
