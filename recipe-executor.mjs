...
function findSelectedCookbookTarget(memoryContext = null, recipeTitle = '') {
  ...
  const cookbookEntries = Array.isArray(memoryContext?.cookbookEntries) ? memoryContext.cookbookEntries.filter(Boolean) : [];
  const matches = cookbookEntries.filter((entry) => normalizeTitleKey(entry?.title) === titleKey);
...
async function resolveRecipeBase({ req, chatId, memoryContext = null, targetRecipe = null }) {
  const explicitTarget = targetRecipe && typeof targetRecipe === 'object' && !Array.isArray(targetRecipe) ? targetRecipe : null;
  ...
  if (explicitTarget?.type === 'cookbook_entry') {
    const cookbookTarget =
      selectedCookbookEntries.find((entry) => Number(entry?.id) === Number(explicitTarget.id)) ||
      (Array.isArray(memoryContext?.cookbookEntries) ? memoryContext.cookbookEntries : []).find((entry) => Number(entry?.id) === Number(explicitTarget.id)) ||
      findSelectedCookbookTarget(memoryContext, safeTrim(explicitTarget.title || explicitTarget.label));
    if (cookbookTarget) {
      return {
        recentConversation: await getMessages(chatId, req.householdId).then((messages) => messages.slice(-16)),
        latestAssistantText: '',
        cookbookTarget,
        baseRecord: buildCookbookRecordForStorage(cookbookTarget),
      };
    }
  }
  if (explicitTarget?.type === 'chat_recipe' && safeTrim(explicitTarget.recipeText)) {
    const parsed = parseCookbookRecipeText(explicitTarget.recipeText, {}) || null;
    if (parsed) {
      const cookbookTarget = findSelectedCookbookTarget(memoryContext, parsed.title || safeTrim(explicitTarget.title));
      return {
        recentConversation: await getMessages(chatId, req.householdId).then((messages) => messages.slice(-16)),
        latestAssistantText: safeTrim(explicitTarget.recipeText),
        cookbookTarget,
        baseRecord: cookbookTarget
          ? buildExplicitCookbookReplacement(cookbookTarget, parsed)
          : buildCookbookRecordForStorage(parsed),
      };
    }
  }
  if (explicitTarget?.type === 'chat_recipe' && explicitTarget.recipeRecord && typeof explicitTarget.recipeRecord === 'object' && !Array.isArray(explicitTarget.recipeRecord)) {
    const parsed = buildCookbookRecordForStorage(explicitTarget.recipeRecord);
    const cookbookTarget = findSelectedCookbookTarget(memoryContext, parsed?.title || safeTrim(explicitTarget.title));
    if (parsed) {
      return {
        recentConversation: await getMessages(chatId, req.householdId).then((messages) => messages.slice(-16)),
...
export async function executeRecipeRevise(runtimeAction, context) {
  const { req, chatId, prompt, anthropic, memoryContext = null } = context;
  const request = safeTrim(runtimeAction?.input?.request || prompt);
  const targetRecipe = runtimeAction?.input?.targetRecipe;
  if (!request) {
    return {
      capability: 'recipe.revise',
      status: 'invalid',
      error: 'Tell me what you want to change in the recipe.',
    };
  }

  const { recentConversation, latestAssistantText, cookbookTarget, baseRecord } = await resolveRecipeBase({
    req,
    chatId,
    memoryContext,
    targetRecipe,
  ...
  if (!baseRecord) {
    return {
      ...
      question: 'Which recipe do you want me to revise?',
      error: 'I need a specific recipe in view before I can revise it.',
    };
  }

  const revisedRecord = await reviseStructuredRecipe({
    anthropic,
    householdId: req.householdId,
    chatId,
    request,
    recipeRecord: baseRecord,
    recentConversation,
    latestAssistantText,
    memoryContext,
  });
  ...
  if (!revisedRecord) {
    return {
      capability: 'recipe.revise',
      status: 'invalid',
      error: 'I could not revise that recipe cleanly yet.',
    };
  }

  const presentableRecord = sanitizeRecipeReplyRecord(revisedRecord) || revisedRecord;
  const unchanged = sameRecipeBody(baseRecord, presentableRecord);
  const title = presentableRecord.title || baseRecord.title;
  const replyText = formatRecipeForReply(presentableRecord);
  const proposedNextAction = !unchanged && Number.isFinite(Number(cookbookTarget?.id))
    ? buildClarifyActionState({
        capability: 'cookbook.update',
        input: {
          id: Number(cookbookTarget.id),
          request,
          revisedRecord: presentableRecord,
        },
        question: `If you want, I can replace ${title} in your cookbook with this revised version.`,
        contextSummary: `Continue the pending cookbook replacement for ${title}.`,
        visibleReplySummary: `If you want, I can replace ${title} in your cookbook with this revised version.`,
      })
    : null;

  return {
    capability: 'recipe.revise',
    status: unchanged ? 'unchanged' : 'revised',
    title,
    revisedRecord,
    replyText,
    proposedNextAction,
