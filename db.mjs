...
async function initializeSchema() {
  ...
  // KitchenBot deliberately has no freeform memory store (removed 2026-07-24). The durable
  // stores are structured + user-visible: household_defaults and person_profiles. Drop the
  // legacy kb_memories table and all of its rows on existing databases; a no-op on fresh ones.
  await run(`DROP TABLE IF EXISTS kb_memories`);
